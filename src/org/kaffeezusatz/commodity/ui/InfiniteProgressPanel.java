...
/**
 * Originally from http://jroller.com/gfx/entry/wait_with_style_in_swing
 */
public class InfiniteProgressPanel extends JComponent implements MouseListener {
	...
	public InfiniteProgressPanel() {
		this("", DEFAULT_BAR_COUNT);
	}

	public InfiniteProgressPanel(String text) {
		this(text, DEFAULT_BAR_COUNT);
	}
	
	public InfiniteProgressPanel(String text, int barsCount) {
		this.text = text;
		this.barsCount = barsCount > 0 ? barsCount : DEFAULT_BAR_COUNT;
	...
	public void paintComponent(Graphics g) {
		if (started) {
			...
			if (text != null && text.length() > 0) {
				...
				layout.draw(g2, (float) (getWidth() - bounds.getWidth()) / 2, (float) (maxY + layout.getLeading() + 2 * layout.getAscent()));
			}
		}
	}
	...
	private Area buildPrimitive() {	
		Rectangle2D.Double body = new Rectangle2D.Double(barWidth/2, 0, barLength, barWidth);
		Ellipse2D.Double head = new Ellipse2D.Double(0, 0, barWidth, barWidth);
		Ellipse2D.Double tail = new Ellipse2D.Double(barLength, 0, barWidth, barWidth);
	...
	public static class Action {
		...
		private class InfiniteProgressPanelThread extends Thread {
			private final Runnable task;
			
			public InfiniteProgressPanelThread(final Runnable task) {
				this.task = task;
				
				setDaemon(true);
				setName("InfiniteProgressPanelThread");
			}
